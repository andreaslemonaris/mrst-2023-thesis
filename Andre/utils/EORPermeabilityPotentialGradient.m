classdef EORPermeabilityPotentialGradient < StateFunction
    % The difference in potential over a face, multiplied with a
    % discretized EOR permeability
    properties
        PermeabilityGradientDiscretization
    end
    
    methods
        function pp = EORPermeabilityPotentialGradient(model, kgrad)
            pp@StateFunction(model);
            % pp.PermeabilityGradientDiscretization = kgrad;
            pp = pp.dependsOn('PhasePotentialDifference');
            pp = pp.dependsOn('AbsolutePermReduction', 'PVTPropertyFunctions');
        ...
        function v = evaluateOnDomain(prop, model, state)
            ...
            v = cell(1, nph);
            perm = prop.getEvaluatedExternals(model, state, 'AbsolutePermReduction');
            for i = 1:nph
