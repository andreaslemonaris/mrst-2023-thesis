...
%}
    ...
    % EQUATIONS ---------------------------------------------------------------
    [pv]               = model.getProps(state, 'PoreVolume');
    [b0, pv0]             = model.getProps(state0, 'PoreVolume');
    ...
    divSurfactant = op.Div(bWvSft-poro.*sW.*D.*op.Div(cs)); %unsure about the operator op.Div
